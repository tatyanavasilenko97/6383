...
public:
    ...
    Queue(const Queue<T> &); // конструктор копировани€
    ~Queue();                // деструктор
...
Queue<T>::Queue(int sizeQueue) :
    size(sizeQueue), // инициализаци€ константы
    begin(0), end(0), elemCT(0)
{
    // дополнительна€ позици поможет нам различать конец и начало очереди
...
// функци€ добавлени€ элемента в очередь
template<typename T>
...
// функци€ удалени€ элемента из очереди
template<typename T>
T Queue<T>::dequeue()
{
    assert( elemCT > 0 );
