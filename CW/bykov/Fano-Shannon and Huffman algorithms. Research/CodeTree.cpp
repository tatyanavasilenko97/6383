...
char* encode(const CodeTree* tree, const char* message)
{
    ...
    long long int len = strlen(message);
	...
	{
        for (long long int i = 0; i < strlen(message); i++)
			code[i] = '0';
...
{
    char* message = new char[MAX_INPUT_LEN];
    long long int index = 0;
    long long int len = strlen(code);
...
void print_tree(CodeTree* tree, long long int level, std::ostream& out)
{
	if (tree)
	{
        ...
        for (long long int i = 0; i < level; i++) out << "   ";
